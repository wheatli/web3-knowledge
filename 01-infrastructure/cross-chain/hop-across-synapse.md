...
2. Router 调 `Pool.swap(...)`：
   ...
   - 扣 `cp.balance -= 10_000e6`，`totalLiquidity -= 10_000e6`。
...
5. Optimism `Bridge.lzReceive(...)`：
   - 先处理 `creditObj`：调本地 `Pool.creditChainPath(Arbitrum_EID, USDC_POOL, credits, idealBalance)`，把 credit 累加进**反方向** ChainPath 的 `balance`，`totalLiquidity += credits`。这一步是 Delta 再平衡的核心——**Arbitrum 的流出在 Optimism 的 Arbitrum-方向车道上转化为可用 balance**。
